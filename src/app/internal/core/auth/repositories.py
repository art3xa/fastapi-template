...
class JWTTokenRepository:
    ...
    async def create(self, user_id: uuid.UUID, jti: str) -> JWTToken:
        jwt_token = JWTToken(jti=jti, user_id=user_id)
    ...
    async def update_by_user_id(self, user_id: uuid.UUID, **kwargs) -> JWTToken:
        stmt = update(JWTToken).where(JWTToken.user_id == user_id).values(**kwargs).returning(JWTToken)
        jwt_token = await self.db_session.execute(stmt)
        return jwt_token.scalars().all()
